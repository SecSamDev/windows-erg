...
fn main() -> windows_erg::Result<()> {
    ...
    println!("  BinaryData: {:02X?}", key.get_value::<Vec<u8>>("BinaryData")?);
    println!("  PathList: {:?}", key.get_value::<Vec<String>>("PathList")?);

    println!("\n✓ All operations successful!");
    println!("\nNote: To clean up, run: reg delete HKCU\\{} /f", test_path);
